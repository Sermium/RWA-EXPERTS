...
import { FormData, FormErrors, UploadedFile, USE_CASES } from '@/types/tokenization';
...
export function Step2Tokenization({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-ink mb-2">Tokenization Details</h2>
      ...
      <div className="p-4 bg-surface-overlay/50 border border-border-strong rounded-xl">
        <div className="flex items-center gap-2 mb-2">
          <Calculator className="w-5 h-5 text-gold-400" />
          <span className="text-ink-muted font-medium">Asset Valuation</span>
        </div>
        <p className="text-2xl font-bold text-ink">
          ${formData.estimatedValue || '0'} <span className="text-ink-muted text-lg">{formData.currency}</span>
        </p>
        <p className="text-ink-faint text-sm mt-1">
          From Step 1 â€¢ This value will be distributed across your tokens
        </p>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {/* Logo Upload - 1:1 */}
          <div>
            <p className="text-xs text-ink-muted mb-2">Logo (1:1)</p>
            {logoFile ? (
              <div className="relative aspect-square bg-surface-overlay rounded-xl overflow-hidden border-2 border-gold-500/50">
                <img src={logoFile.url} alt="Logo" className="w-full h-full object-cover" />
                <button
                  onClick={removeLogo}
                  className="absolute top-2 right-2 p-1.5 bg-danger rounded-full hover:bg-danger transition-colors"
                >
              ...
              <button
                ...
                className="w-full aspect-square bg-surface-overlay/50 border-2 border-dashed border-border-strong rounded-xl flex flex-col items-center justify-center hover:border-gold-500 hover:bg-surface-overlay transition-all"
              >
                {uploadingLogo ? (
                  <Loader2 className="w-6 h-6 text-gold-400 animate-spin" />
                ) : (
                  <>
                    <Image className="w-6 h-6 text-ink-muted mb-1" />
                    <span className="text-xs text-ink-muted">Upload</span>
                  </>
                )}
              </button>
            )}
          ...
          <div className="col-span-3">
            <p className="text-xs text-ink-muted mb-2">Banner (3:1)</p>
            {bannerFile ? (
              <div className="relative aspect-[3/1] bg-surface-overlay rounded-xl overflow-hidden border-2 border-gold-500/50">
              ...
              <button
                ...
                className="w-full aspect-[3/1] bg-surface-overlay/50 border-2 border-dashed border-border-strong rounded-xl flex flex-col items-center justify-center hover:border-gold-500 hover:bg-surface-overlay transition-all"
              >
                {uploadingBanner ? (
                  <Loader2 className="w-6 h-6 text-gold-400 animate-spin" />
                ) : (
                  <>
                    <Image className="w-6 h-6 text-ink-muted mb-1" />
                    <span className="text-xs text-ink-muted">Upload Banner</span>
                  </>
                )}
              </button>
            )}
      ...
      <div>
        ...
        <div className="relative">
          <Coins className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-muted" />
          <input
            type="text"
        ...
        {errors.tokenName && (
          <p className="mt-1 text-sm text-danger">{errors.tokenName}</p>
        )}
      </div>
      ...
      <div>
        ...
        <div className="relative">
          <Tag className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-muted" />
          <input
            type="text"
            ...
            placeholder="e.g., DOT"
            maxLength={10}
            className={`w-full pl-10 pr-4 py-3 bg-surface-overlay border rounded-xl text-ink placeholder-ink-faint focus:outline-none focus:ring-2 focus:ring-gold-500 uppercase ${
              errors.tokenSymbol ? 'border-danger' : 'border-border-strong'
            }`}
          />
        </div>
        {errors.tokenSymbol && (
          <p className="mt-1 text-sm text-danger">{errors.tokenSymbol}</p>
        )}
      ...
      <div>
        ...
        <div className="relative">
          <Hash className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-muted" />
          <input
            type="text"
        ...
        {errors.totalSupply && (
          <p className="mt-1 text-sm text-danger">{errors.totalSupply}</p>
        )}
        <p className="mt-1 text-xs text-ink-faint">
      ...
      <div>
        ...
        <div className="relative">
          <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-muted" />
          <input
            type="text"
            value={pricePerToken > 0 ? `$${pricePerToken.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 4 })}` : '-'}
            readOnly
            className="w-full pl-10 pr-20 py-3 bg-border-strong/50 border border-border-strong rounded-xl text-ink cursor-not-allowed"
          />
      ...
      <div>
        <label className="block text-sm font-medium text-ink-muted mb-2">
          Primary Use Case <span className="text-ink-faint">(Optional)</span>
        </label>
        <div className="relative">
          <Target className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-muted" />
          <select
            value={formData.useCase}
            onChange={(e) => updateFormData('useCase', e.target.value)}
            className="w-full pl-10 pr-4 py-3 bg-surface-overlay border border-border-strong rounded-xl text-ink focus:outline-none focus:ring-2 focus:ring-gold-500 appearance-none"
          >
      ...
      {formData.tokenName && formData.totalSupply && totalSupply > 0 && (
        <div className="mt-6 overflow-hidden rounded-xl border border-gold-500/30">
          {/* Banner Preview - 3:1 */}
          {bannerFile ? (
            <div className="relative aspect-[3/1] bg-surface-overlay">
              <img src={bannerFile.url} alt="Banner" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-surface-sunken/80 to-transparent" />
            </div>
          ) : (
          ...
          <div className="p-4 bg-gradient-to-r from-gold-500/10 to-gold-light-500/10 -mt-12 relative">
            <div className="flex items-end gap-4 mb-4">
              {logoFile ? (
                <img 
                  src={logoFile.url} 
                  alt="Logo" 
                  className="w-20 h-20 rounded-xl object-cover border-4 border-border-strong shadow-lg" 
                />
              ) : (
                <div className="w-20 h-20 rounded-xl bg-surface-overlay flex items-center justify-center border-4 border-border-strong shadow-lg">
                  <Coins className="w-8 h-8 text-ink-faint" />
                </div>
              )}
              <div className="pb-1">
                <p className="text-ink font-semibold text-lg">{formData.tokenName}</p>
            ...
            <div className="grid grid-cols-3 gap-3 text-sm">
              <div className="bg-surface/50 rounded-lg p-3 text-center">
                <span className="text-ink-muted text-xs block">Supply</span>
                <p className="text-ink font-medium">{formData.totalSupply}</p>
              </div>
              <div className="bg-surface/50 rounded-lg p-3 text-center">
                <span className="text-ink-muted text-xs block">Price</span>
                <p className="text-ink font-medium">
                  ${pricePerToken.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 4 })}
                </p>
              </div>
              <div className="bg-surface/50 rounded-lg p-3 text-center">
                <span className="text-ink-muted text-xs block">Valuation</span>
