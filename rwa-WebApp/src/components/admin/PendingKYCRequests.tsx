...
export default function PendingKYCRequests() {
  ...
  const renderDocumentPreview = (url: string | null, label: string, type: string) => {
    ...
    return (
      <div className="border rounded-lg p-3 bg-surface">
        ...
        >
          {isPDF ? (
            <div className="w-full h-32 bg-surface-overlay rounded flex items-center justify-center">
              <div className="text-center">
                <svg className="w-12 h-12 mx-auto text-ink-faint" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                </svg>
                <span className="text-sm text-ink-faint">PDF Document</span>
              </div>
            </div>
          ) : (
            <img 
        ...
        <button
          ...
          className="mt-2 text-xs text-gold-600 hover:text-gold-800"
        >
          Open in new tab
        </button>
      </div>
      ...
      >
        <div className="flex justify-between items-start mb-3">
          <div>
            ...
            <p className="text-xs text-ink-faint">{formatDate(submission.submittedAt)}</p>
          </div>
          <span className={`px-2 py-1 rounded text-xs text-ink ${
        ...
        <div className="flex items-center gap-2 mb-3">
          <span className={`px-2 py-1 rounded text-xs text-ink ${TIER_COLORS[currentTier]}`}>
            {currentTier}
          </span>
          <svg className="w-4 h-4 text-ink-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
          </svg>
          <span className={`px-2 py-1 rounded text-xs text-ink ${TIER_COLORS[requestedTier]}`}>
            {requestedTier}
          </span>
        </div>
        
        {submission.personalInfo?.fullName && (
          <p className="text-sm text-ink-muted">{submission.personalInfo.fullName}</p>
        )}
        
        <div className="mt-2 flex gap-1 flex-wrap">
          {submission.documents.hasIdDocument && (
            <span className="px-2 py-0.5 bg-gold-100 text-gold-700 text-xs rounded">ID</span>
          )}
          {submission.documents.hasSelfie && (
            <span className="px-2 py-0.5 bg-success/10 text-success text-xs rounded">Selfie</span>
          )}
          {submission.documents.hasAddressProof && (
            <span className="px-2 py-0.5 bg-warning/10 text-warning text-xs rounded">Address</span>
          )}
          {submission.documents.hasAccreditedProof && (
            <span className="px-2 py-0.5 bg-gold-100 text-gold-700 text-xs rounded">Accredited</span>
          )}
        </div>
  ...
  const renderDetailView = () => {
    ...
    return (
      <div className="border rounded-lg bg-surface p-6">
        <div className="flex justify-between items-start mb-6">
          <div>
            <h3 className="text-lg font-semibold">
              {selectedSubmission.isUpgrade ? 'Upgrade Request' : 'New Submission'}
            </h3>
            <p className="font-mono text-sm text-ink-faint">{selectedSubmission.walletAddress}</p>
          </div>
          <button 
            onClick={() => setSelectedSubmission(null)}
            className="text-ink-muted hover:text-ink-faint"
          >
        ...
        <div className="mb-6 p-4 bg-surface rounded-lg">
          <p className="text-sm text-ink-faint mb-2">Tier Change</p>
          <div className="flex items-center gap-3">
            <span className={`px-3 py-1.5 rounded text-ink font-medium ${TIER_COLORS[currentTier]}`}>
              {currentTier}
            </span>
            <svg className="w-6 h-6 text-ink-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
            </svg>
            <span className={`px-3 py-1.5 rounded text-ink font-medium ${TIER_COLORS[requestedTier]}`}>
              {requestedTier}
            </span>
          </div>
        </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              {selectedSubmission.validationScores.idValidationConfidence !== undefined && (
                <div className="p-3 bg-surface rounded-lg text-center">
                  ...
                  <p className="text-xs text-ink-faint">ID Confidence</p>
              ...
              {selectedSubmission.validationScores.faceScore !== undefined && (
                <div className="p-3 bg-surface rounded-lg text-center">
                  ...
                  <p className="text-xs text-ink-faint">Face Score</p>
                </div>
              )}
              {selectedSubmission.validationScores.livenessScore !== undefined && (
                <div className="p-3 bg-surface rounded-lg text-center">
                  ...
                  <p className="text-xs text-ink-faint">Liveness</p>
                  <span className={`inline-block mt-1 px-2 py-0.5 rounded text-xs ${
        ...
        <div className="mb-6">
          ...
          {!selectedSubmission.documents.hasIdDocument && 
           ...
           !selectedSubmission.documents.hasAccreditedProof && (
            <p className="text-ink-faint text-sm">No documents submitted</p>
          )}
        </div>
        ...
        <div className="flex gap-3 pt-4 border-t">
          <button
            ...
            disabled={processing}
            className="flex-1 bg-success text-ink py-2 px-4 rounded-lg hover:bg-success/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
          ...
          <button
            ...
            disabled={processing}
            className="flex-1 bg-danger text-ink py-2 px-4 rounded-lg hover:bg-danger/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
    ...
    return (
      <div className="p-6 text-center">
        ...
        <button 
          ...
          className="px-4 py-2 bg-gold-600 text-ink rounded hover:bg-gold-700"
        >
          Retry
        </button>
      </div>
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          {selectedSubmission ? (
            ...
            <div className="border rounded-lg p-8 text-center text-ink-faint">
      ...
      {documentModal && (
        <div 
          className="fixed inset-0 bg-surface-sunken/75 flex items-center justify-center z-50 p-4"
          onClick={() => setDocumentModal(null)}
        >
          <div 
            className="bg-surface rounded-lg max-w-4xl max-h-[90vh] overflow-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-4 border-b flex justify-between items-center sticky top-0 bg-surface">
              ...
              <div className="flex gap-2">
                <a
                  ...
                  className="px-3 py-1 bg-gold-600 text-ink rounded text-sm hover:bg-gold-700"
                >
                  Open in new tab
                </a>
                <button 
