    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
      <div className="bg-surface border border-border rounded-xl p-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-ink-muted">Total Deals</span>
          <FileText className="h-5 w-5 text-gold-400" />
        </div>
        <p className="text-2xl font-bold text-ink">{stats.totalDeals}</p>
        <p className="text-xs text-ink-faint">{stats.activeDeals} active</p>
      </div>
      ...
      <div className="bg-surface border border-border rounded-xl p-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-ink-muted">Disputed</span>
          <AlertTriangle className="h-5 w-5 text-danger" />
        </div>
        <p className="text-2xl font-bold text-ink">{stats.disputedDeals}</p>
        <p className="text-xs text-ink-faint">
      ...
      <div className="bg-surface border border-border rounded-xl p-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-ink-muted">Avg. Deal Size</span>
          <TrendingUp className="h-5 w-5 text-gold-400" />
        </div>
        <p className="text-2xl font-bold text-ink">
          ${(stats.averageDealSize / 1_000).toFixed(0)}K
        </p>
        <p className="text-xs text-ink-faint">{stats.completedDeals} completed</p>
      </div>
    </div>
  );
}
...
function DealRow({ 
  ...
  const getRiskColor = () => {
    if (deal.riskScore >= 80) return 'text-danger';
    if (deal.riskScore >= 50) return 'text-warning';
    return 'text-success';
  };

  const getRiskBgColor = () => {
    if (deal.riskScore >= 80) return 'bg-danger';
    if (deal.riskScore >= 50) return 'bg-warning';
    return 'bg-success';
  };
    ...
    <tr className="border-b border-border hover:bg-surface/30">
      <td className="py-4 px-4">
        <div className="flex items-center">
          {deal.flagged && (
            <AlertTriangle className="h-4 w-4 text-danger mr-2 flex-shrink-0" />
          )}
          <div>
            <p className="text-ink font-medium">{deal.reference}</p>
            <p className="text-xs text-ink-muted truncate max-w-[200px]">{deal.title}</p>
          </div>
        </div>
      </td>
      ...
      <td className="py-4 px-4">
        ...
        <p className="text-xs text-ink-muted">{deal.currency}</p>
      </td>
      ...
      <td className="py-4 px-4">
        <div className="flex items-center">
          <div className="w-16 h-2 bg-surface-overlay rounded-full overflow-hidden mr-2">
            <div 
        ...
        <div className="flex items-center gap-2 relative">
          ...
          <button
            ...
            className={`p-2 transition-colors ${
              deal.flagged ? 'text-danger hover:text-danger' : 'text-ink-muted hover:text-warning'
            }`}
          ...
          <div className="relative">
            ...
            {showMenu && (
              <>
                ...
                />
                <div className="absolute right-0 top-full mt-1 w-48 bg-surface rounded-lg border border-border shadow-xl z-20">
                  <button
                    onClick={() => {
                      onUpdateKyc('buyer', 'silver');
                      setShowMenu(false);
                    }}
                    className="w-full px-4 py-2 text-left text-sm text-ink-muted hover:bg-surface-overlay flex items-center"
                  >
                  ...
                  <button className="w-full px-4 py-2 text-left text-sm text-ink-muted hover:bg-surface-overlay flex items-center">
                  ...
                  <button className="w-full px-4 py-2 text-left text-sm text-danger hover:bg-surface-overlay flex items-center">
...
function DealDetailModal({
  ...
  return (
    <div className="fixed inset-0 bg-surface-sunken/50 flex items-center justify-center z-50 p-4">
      <div className="bg-surface rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
        <div className="p-6 border-b border-border flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-ink">{deal.reference}</h2>
            <p className="text-ink-muted">{deal.title}</p>
          </div>
          <button
            onClick={onClose}
            className="p-2 text-ink-muted hover:text-ink transition-colors"
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-surface-sunken/50 rounded-xl p-4 border border-border/50">
              <h3 className="text-sm font-medium text-ink-muted mb-3 flex items-center">
                <Users className="h-4 w-4 mr-2" />
                Buyer
              </h3>
              <p className="text-ink font-medium">{deal.buyerCompany}</p>
              <p className="text-sm text-ink-muted">{COUNTRY_NAMES[deal.buyerCountry] || deal.buyerCountry}</p>
              <p className="text-xs text-ink-faint mt-2 font-mono truncate">{deal.buyerWallet}</p>
              
              <div className="mt-4 pt-4 border-t border-border">
                <label className="block text-sm text-ink-muted mb-2">KYC Level</label>
                <select
                  value={buyerKyc}
            ...
            <div className="bg-surface-sunken/50 rounded-xl p-4 border border-border/50">
              ...
              <p className="text-ink font-medium">{deal.sellerCompany}</p>
              <p className="text-sm text-ink-muted">{COUNTRY_NAMES[deal.sellerCountry] || deal.sellerCountry}</p>
              <p className="text-xs text-ink-faint mt-2 font-mono truncate">{deal.sellerWallet}</p>
            ...
            <div className="bg-surface-sunken/50 rounded-xl p-4 border border-border/50">
              ...
              <div className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="text-ink">{deal.productName}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-ink-muted">Category</span>
                  <span className="text-ink capitalize">{deal.productCategory.replace('_', ' ')}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-ink-muted">Value</span>
                  <span className="text-ink font-medium">${deal.totalValue.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-ink-muted">Incoterm</span>
                  <span className="text-ink">{deal.incoterm}</span>
                </div>
              </div>
            </div>
            ...
            {/* Status & Actions */}
            <div className="bg-surface-sunken/50 rounded-xl p-4 border border-border/50">
              <h3 className="text-sm font-medium text-ink-muted mb-3">Stage Management</h3>
              <select
                value={selectedStage}
                onChange={(e) => setSelectedStage(e.target.value)}
                className="w-full px-3 py-2 bg-surface border border-border rounded-lg text-ink mb-4"
              >
              ...
              <div className="pt-4 border-t border-border">
                <p className="text-sm text-ink-muted mb-2">Risk Score</p>
                <div className="flex items-center">
                  <div className="flex-1 h-3 bg-surface-overlay rounded-full overflow-hidden mr-3">
                    <div 
                      className={`h-full ${
                        deal.riskScore >= 80 ? 'bg-danger' :
                        deal.riskScore >= 50 ? 'bg-warning' : 'bg-success'
                      }`}
                      style={{ width: `${deal.riskScore}%` }}
                    />
                  </div>
                  <span className={`text-lg font-bold ${
                    deal.riskScore >= 80 ? 'text-danger' :
                    deal.riskScore >= 50 ? 'text-warning' : 'text-success'
                  }`}>
                    {deal.riskScore}
                  </span>
                </div>
          ...
          <div className="mt-6 bg-surface-sunken/50 rounded-xl p-4 border border-border/50">
            <h3 className="text-sm font-medium text-ink-muted mb-3 flex items-center">
        ...
        <div className="p-6 border-t border-border flex justify-end gap-3">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-surface-overlay text-ink rounded-lg hover:bg-border-strong transition-colors"
          >
            Cancel
          </button>
          <button 
            onClick={handleSave}
            disabled={isSaving}
            className="px-4 py-2 bg-gold-500 text-ink rounded-lg hover:bg-gold-600 transition-colors disabled:opacity-50 flex items-center"
          >
            {isSaving ? (
              <>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        ...
        <div className="flex gap-2">
          <button 
            ...
            className="px-4 py-2 bg-surface text-ink rounded-lg hover:bg-surface-overlay transition-colors flex items-center border border-border"
          >
          ...
          <button 
            ...
            className="p-2 bg-surface text-ink rounded-lg hover:bg-surface-overlay transition-colors border border-border"
          >
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-surface-sunken/50 border-b border-border">
                <th className="text-left py-3 px-4 text-sm font-medium text-ink-muted">Deal</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-ink-muted">Buyer</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-ink-muted">Seller</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-ink-muted">Value</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-ink-muted">Stage</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-ink-muted">Risk</th>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="py-12 text-center text-ink-muted">
                    <Ship className="h-12 w-12 text-ink-faint mx-auto mb-3" />
                    <p>No deals found</p>
                    <p className="text-sm text-ink-faint mt-1">Try adjusting your filters</p>
                  </td>
                </tr>
              ) : (
                deals.map((deal) => (
          ...
          <div className="p-4 border-t border-border flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={page === 1}
                className="p-2 bg-surface-overlay text-ink rounded-lg disabled:opacity-50 hover:bg-border-strong transition-colors"
              >
