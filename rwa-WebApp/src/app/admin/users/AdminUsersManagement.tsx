...
function NetworkBadge({ 
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 bg-surface-overlay/50 rounded-lg">
      <div className={`w-2 h-2 rounded-full ${isTestnet ? 'bg-warning' : 'bg-green-400'}`} />
...
export default function AdminUsersManagement() {
  ...
  return (
    <div className="space-y-6">
      {/* Network Info Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-surface/50 rounded-xl p-4 border border-border">
        <div className="flex items-center gap-3">
          <Globe className="w-5 h-5 text-gold-400" />
          <div>
            <div className="text-sm text-ink-muted">Connected Network</div>
        ...
        {deployedChains.length > 1 && (
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-sm text-ink-faint">Switch to:</span>
        ...
        <div className="bg-surface rounded-xl p-4 border border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-ink-muted text-sm">Total Admins</div>
            </div>
          </div>
        </div>
        <div className="bg-surface rounded-xl p-4 border border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-2xl font-bold text-ink">{superAdminCount}</div>
              <div className="text-ink-muted text-sm">Super Admins</div>
            </div>
          </div>
        </div>
        <div className="bg-surface rounded-xl p-4 border border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-ink-muted text-sm">Regular Admins</div>
            </div>
          </div>
        </div>
        <div className="bg-surface rounded-xl p-4 border border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-2xl font-bold text-ink">{deployedChains.length}</div>
              <div className="text-ink-muted text-sm">Networks</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {isSuperAdmin && (
        <div className="bg-surface border border-border rounded-xl p-6">
          <h2 className="text-lg font-semibold text-ink mb-4 flex items-center gap-2">
            ...
            Add New Admin
            <span className="text-sm font-normal text-ink-faint">on {chainName}</span>
          </h2>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  placeholder="0x..."
                  className="w-full px-4 py-2.5 bg-surface-overlay border border-border-strong rounded-lg text-ink placeholder-ink-faint focus:border-gold-500 focus:outline-none font-mono text-sm"
                />
              </div>
              <div>
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 bg-surface-overlay border border-border-strong rounded-lg text-ink placeholder-ink-faint focus:border-gold-500 focus:outline-none text-sm w-48 sm:w-64"
              />
            </div>
            <button
              onClick={refreshAdmins}
              className="p-2 bg-surface-overlay hover:bg-border-strong rounded-lg text-ink-muted hover:text-ink transition-colors"
              title="Refresh"
            >
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-sm text-ink-faint mt-1">
                        ...
                        <>
                          {isSuperAdminUser ? (
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 px-3 py-1.5 text-sm text-warning hover:bg-warning/10 rounded-lg transition-colors"
            ...
            <div className="border-t border-border">
              {loadingActivityLog ? (
                <div className="p-8 text-center">
                  ...
                  <p className="text-ink-faint mt-2">Loading activity from {chainName}...</p>
                </div>
                ...
                <div className="divide-y divide-border">
                  {activityLog.map((log) => (
                    <div key={log.id} className="p-4">
                      ...
                      <div className="flex items-center gap-2 text-sm text-ink-faint mt-1">
      ...
      <div className="bg-surface/30 border border-border rounded-xl p-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-sm">
          <div className="flex items-center gap-4 text-ink-faint">
            ...
            <span>•</span>
            <span>Currency: <span className="text-ink-muted">{nativeCurrency?.symbol || 'ETH'}</span></span>
          </div>
          {explorerUrl && (
            <a
              href={explorerUrl}
      ...
      {!isSuperAdmin && (
        <div className="bg-surface/50 border border-border rounded-xl p-6 text-center">
