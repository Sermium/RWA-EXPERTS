// scripts/prepare-verification.ts
...
function main() {
  ...
  const buildInfo = JSON.parse(fs.readFileSync(path.join(buildInfoDir, jsonFile), "utf-8"));
  ...
  fs.writeFileSync("verification-source.json", sourceCode);
  console.log(`Source code saved to verification-source.json (${sourceCode.length} bytes)`);
  console.log(`Compiler version: ${compilerVersion}`);

  // Save contracts list
  fs.writeFileSync("verification-contracts.json", JSON.stringify({ contracts, compilerVersion }, null, 2));
  console.log("Contracts list saved to verification-contracts.json");
}
